...
public class TextUtil {
	...
	public static String formatDoubleTo2(Object number){
		try{
			...
			Logger.t(TAG).e("格式化出错：" + e.getMessage());
		}
		return "0.00";
	}
	...
	public static BigDecimal createBigDecimal(String number){
		try {
			...
			e.printStackTrace();
			Logger.t(TAG).e("BigDecimal格式化错误了：" + number);
		}
	...
	public static void setPrice(TextView textView, String price, String defPrice){
		String priceStr = String.format(Locale.CHINA,"¥%1$s",isEmpty(price) ? defPrice : price);
		textView.setText(priceStr);
	}

	public static String getPriceStr(String price, String defPrice){
		return String.format(Locale.CHINA,"¥%1$s",isEmpty(price) ? defPrice : price);
	}
	...
	*/
	...
	public static String inputStream2StringFromGZIP(InputStream is) {
		...
		try {
			...
			FileUtil.closeIO(is, bis, reader);
		}
