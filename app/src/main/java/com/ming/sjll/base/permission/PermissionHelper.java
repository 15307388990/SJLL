...
public class PermissionHelper {
    ...
    public static final int REQ_PERMISSION_LOCATION = 9;//定位
     ...
     * @param listener
     */
    public static void reqLocaiton(final Activity activity, final PermissionListener listener) {
        XPermissionUtils.requestPermissions(activity, REQ_PERMISSION_LOCATION, new String[]{
                ...
                new XPermissionUtils.OnPermissionListener() {
                    ...
                    @Override
                    public void onPermissionDenied(final String[] deniedPermissions, boolean alwaysDenied) {
                        ToastShow.s("获取定位权限失败");
                        if (listener != null) {
                            listener.onFailure();
                        }
                        if (alwaysDenied) { // 拒绝后不再询问 -> 提示跳转到设置
                            PermissionDialogUtil.showPermissionManagerDialog(activity, "定位");
                        } else {    // 拒绝 -> 提示此公告的意义，并可再次尝试获取权限
                            new AlertDialog.Builder(activity).setTitle("温馨提示")
